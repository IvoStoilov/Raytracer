...
{
	float ratio = (float)x / SCREEN_WIDTH;
	vec4 _x = MathUtil::Lerp(m_TopLeft, m_TopRight, ratio);
	ratio = (float)y / SCREEN_HEIGHT;
	vec4 screenPos = MathUtil::Lerp(_x, m_BottomRight, ratio);

	vec4 rayDirection = screenPos - m_Position;
